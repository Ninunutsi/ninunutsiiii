...
const DetailedProductPage = () => {
  const { productId } = useParams();
    const {productId} = useParams()
    const images = [ popularCollection1, popularCollection2, popularCollection1, popularCollection2, popularCollection1, popularCollection2]
    return(
        <div>
            {products?.map(prod => (
                prod.id === productId &&
                    <div className="detailed-product-container" key={prod.id}>
                        <div className="detailed-slider"><PhotoSwiper photos={prod.moreImages}/></div>
